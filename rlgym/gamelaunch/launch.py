...
class ROCKET_LEAGUE_PROCESS_INFO:
    ...
    def get_ideal_args(pipe_id):
        ...
        # -noeac (lowercase!) launches Rocket League without Easy Anti-Cheat so the injector can attach
        # (RL added mandatory EAC in Season 22, Apr 2026; EAC blocks DLL injection). Belt-and-suspenders
        # with the Epic Launcher "Launch options: -noeac" step -- on Epic, EAC is decided by the LAUNCHER,
        # so also set it there. -noeac disables online play; fine for this offline/injector use.
        return ['-pipe', f'{pipe_id}', '-nomovie', '-noeac']
